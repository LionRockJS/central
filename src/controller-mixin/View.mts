...
export enum ControllerMixinViewState {
...
export default class ControllerMixinView extends ControllerMixin {
  ...
  static override init(state) {
    ...
    if (!state.get(ControllerMixinViewState.LAYOUT_FILE))state.set(ControllerMixinViewState.LAYOUT_FILE, 'layout/default');
    if (!state.get(ControllerMixinViewState.PLACEHOLDER))state.set(ControllerMixinViewState.PLACEHOLDER, 'main');
    if (!state.get(ControllerMixinViewState.VIEW_CLASS))state.set(ControllerMixinViewState.VIEW_CLASS, View.DefaultViewClass);
    if (!state.get(ControllerMixinViewState.LAYOUT_DEFAULT_DATA))state.set(ControllerMixinViewState.LAYOUT_DEFAULT_DATA, defaultLayoutData);
    if (!state.get(ControllerMixinViewState.VIEW_DEFAULT_DATA))state.set(ControllerMixinViewState.VIEW_DEFAULT_DATA, defaultViewData);
    if (!state.get(ControllerMixinViewState.LAYOUT)){
      this.setLayout(state,
        state.get(ControllerMixinViewState.LAYOUT_FILE),
        state.get(ControllerMixinViewState.LAYOUT_DEFAULT_DATA),
        'layout/default'
      );
    }
  }
