...
describe('HelperCrypto Source', () => {
  test('instantiation', () => {
    new HelperCrypto();
  });

  test('make key', async () => {
    // We'll write to a file in the keys directory we just created
    const fileName = path.join(__dirname, 'keys', 'testKeySource.js');
    const jwk = await HelperCrypto.makeSignKey(fileName, { name: 'HMAC', hash: 'SHA-256' });
    
    // Dynamic import of the generated file
    // Note: In Bun/Node, importing the same file again might be cached. 
    // Since this is a new test file and new filename, it should be fine.
    const {default : key} = await import('./keys/testKeySource.js');
    ...
    const fileName2 = path.join(__dirname, 'keys', 'testKeySource2.js');
    const jwk2 = await HelperCrypto.makeSignKey(fileName2);
    const {default: key2} = await import('./keys/testKeySource2.js');
