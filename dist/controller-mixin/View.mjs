...
export default class ControllerMixinView extends ControllerMixin {
    ...
    static init(state) {
        ...
        if (!state.get(this.LAYOUT_FILE))
            state.set(this.LAYOUT_FILE, 'layout/default');
        if (!state.get(this.PLACEHOLDER))
            state.set(this.PLACEHOLDER, 'main');
        if (!state.get(this.VIEW_CLASS))
            state.set(this.VIEW_CLASS, View.DefaultViewClass);
        if (!state.get(this.LAYOUT_DEFAULT_DATA))
            state.set(this.LAYOUT_DEFAULT_DATA, defaultLayoutData);
        if (!state.get(this.VIEW_DEFAULT_DATA))
            state.set(this.VIEW_DEFAULT_DATA, defaultViewData);
        if (!state.get(this.LAYOUT)) {
            this.setLayout(state, state.get(this.LAYOUT_FILE), state.get(this.LAYOUT_DEFAULT_DATA), 'layout/default');
        }
    }
    ...
    static async before(state) {
        if (!state.get(this.LAYOUT))
            this.setLayout(state, state.get(this.LAYOUT_FILE), {});
    }
    ...
    static isSkipLayout(state) {
        ...
        if (!mime)
            return false;
        if (state.get(Controller.STATE_BODY) === null)
            return false;
        if (/\/json/.test(mime))
            return false;
        if (/^text/.test(mime))
    ...
    static async exit(state) {
        if (state.get(Controller.STATE_STATUS) === 302)
            return;
        this.assignJSONView(state);
