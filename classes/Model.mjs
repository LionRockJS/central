...
export default class Model {
  ...
  constructor(id = null, options = {}) {
    ...
    const ConcreteModel = this.constructor;
  ...
  async eagerLoad(option = {}) {
    ...
    * */
    ...
    const props = [];
    await Promise.all(this.constructor.hasMany.map(async (x) => {
      const k = x[0];
      const v = x[1];
      if (!allowClasses.has(v))
        return;
    ...
    await Promise.all(props.map(async (p) => {
      ...
      if (p.opt) {
        await Promise.all(instances.map(async (instance) => instance.eagerLoad(p.opt)));
      }
    }));
    const siblings = [];
    await Promise.all([...this.constructor.belongsToMany.keys()].map(async (x) => {
      if (!allowClasses || !allowClasses.has(x))
        return;
    ...
    await Promise.all(siblings.map(async (s) => {
      ...
      if (s.opt) {
        await Promise.all(instances.map(instance => instance.eagerLoad(s.opt)));
      }
    }));
  }
  /**
  ...
  async #readByValues(columns) {
    ...
    if (values.size === 0)
      throw new Error(`${this.constructor.name}: No id and no value to read`);
  ...
  async delete() {
    if (!this.id)
      throw new Error('ORM delete Error, no id defined');
  ...
  async children(fk, MClass = null) {
    ...
    if (modelNames.length > 1 && MClass === null)
      throw new Error('children fk have multiple Models, please specific which Model will be used');
  ...
  async add(model, weight = 0) {
    if (!this.id)
      throw new Error(`Cannot add ${model.constructor.name}. ${this.constructor.name} not have id`);
    // check model is not empty
    if (!model)
      throw new Error('Error add model, model cannot be null or undefined');
    if (Array.isArray(model) && model.length <= 0)
      throw new Error('Error add model, model array cannot be empty');
  ...
  async remove(model) {
    if (!this.id)
      throw new Error(`Cannot remove ${model.constructor.name}. ${this.constructor.name} not have id`);
    const { joinTableName, lk, fk } = this.#siblingInfo(model);
  ...
  async removeAll(MClass) {
    if (!this.id)
      throw new Error(`Cannot remove ${MClass.name}. ${this.constructor.name} not have id`);
